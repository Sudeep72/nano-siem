...
NanoSIEM v5.0 — Analyst Operations Edition.

This is the final planned release. v5 closes the loop opened by v3
(operations visibility) and v4 (AI reasoning): analysts can now triage,
own, and disposition incidents — and false-positive dispositions feed
back into the ML baseline. Tests: 485 (was 428).

### Added

**Incident Lifecycle — `incidents/model.py`, `incidents/store.py`**
- `Incident` dataclass with state machine: new → triaging → contained → closed,
  with a dismissed branch from new/triaging
- `Disposition` enum: true_positive, false_positive, benign_true_positive, undetermined
- `IncidentNote` — analyst notes with author/timestamp
- `incident_from_alert()` / `incident_from_alerts()` — create incidents from
  one or many correlated alerts, merging severity (max), techniques, source IPs
- `IncidentStore` — in-memory store with create/list/filter/update operations
- API: `POST /api/incidents`, `GET /api/incidents`, `GET /api/incidents/{id}`,
  `PATCH /api/incidents/{id}`, `GET /api/incidents/stats/summary`

**FP Feedback Loop — `incidents/store.py`, `ml/scorer.py`**
- Marking an incident `false_positive` records alert fingerprints
- `IncidentStore` accumulates fingerprints; once `fp_retrain_threshold` (default 5)
  is reached, triggers `AnomalyScorer.retrain_excluding_fingerprints()`
- `AnomalyScorer.retrain_excluding_fingerprints()` — retrains the Isolation
  Forest baseline, validates the new model isn't degraded (≥90% of requested
  training samples) before swapping; keeps previous model on failure/degradation
- Conservative by design: rule-level FPs need a rule edit; instance-level FPs
  feed the retraining loop — these are intentionally different remediation paths

**Dashboard — 4 new tabs (9 total)**
- **Threat Map** — Leaflet.js world map (CartoDB dark tiles, no API key),
  source IPs plotted by geolocation, severity-colored pulsing markers,
  click for enrichment popup (country, ISP, abuse score)
- **Incidents** — full lifecycle UI: create from selected alerts, state
  transition buttons, owner assignment, disposition selector (false_positive
  shows "→ ML retraining queued"), analyst notes thread
- **Knowledge Graph** — canvas-based force-directed graph (no D3 dependency),
  draggable nodes, click for entity description via `/api/graph/{entity_id}`,
  color-coded by node type (source_ip/host/alert/technique/tactic/chain)
- **Replay** — step-through attack replay UI, step navigator, optional AI
  commentary per step + overall threat narrative

**Enrichment badges (Alerts tab)**
- Every alert card with a public source IP shows inline badges: country code,
  AbuseIPDB score (color-coded by risk level), hosting/datacenter flag
- Mini threat map embedded above the alert feed (collapsible)

### New API endpoints
- `POST  /api/incidents` — create incident from alert IDs
- `GET   /api/incidents` — list incidents (filter by state/owner)
- `GET   /api/incidents/{id}` — get single incident
- `PATCH /api/incidents/{id}` — update state/owner/notes/disposition
- `GET   /api/incidents/stats/summary` — feedback loop stats

### New tests (57, total 485)
- `tests/test_incidents.py` — 39 tests: state machine, CRUD, FP feedback loop
- `tests/test_ml_feedback.py` — 9 tests: retrain_excluding_fingerprints,
  degraded-model rejection, exception handling, end-to-end feedback integration

### Frontend dependencies
- Leaflet 1.9.4 (CDN, no build dependency) for Threat Map
- Knowledge Graph uses vanilla canvas + custom force simulation (no D3)
