...
class AnomalyScorer:
    ...
    def retrain_excluding_fingerprints(self, fingerprints: list[str]) -> bool:
        """
        FP feedback loop: retrain the baseline model, recording which
        event fingerprints were false positives so they can be noted.

        In this implementation, the baseline is retrained on a fresh
        synthetic corpus (the same approach as startup training) because
        NanoSIEM's Isolation Forest is trained on synthetic normal-traffic
        data rather than a labeled historical event store.

        In a production system with a persistent event store, this would:
          1. Pull all stored events EXCEPT those matching `fingerprints`
          2. Retrain on that filtered set
          3. Compare AUC before/after and only swap if improved

        For NanoSIEM's scope: retrain from corpus, log the fingerprints
        for audit, and update the threshold slightly toward fewer FPs.

        Returns True if retraining succeeded, False if it should be skipped.
        """
        try:
            if self._model is None:
                logger.warning("FP retrain called but model not loaded — skipping")
                return False

            logger.info(
                "FP-triggered retraining — %d false positive fingerprints recorded: %s",
                len(fingerprints), fingerprints[:5],
            )

            # Retrain on fresh corpus (conservative: don't lower the model quality)
            new_model = train_from_corpus(
                path=self._model_path,
                n_samples=self._train_n_samples,
            )

            # Only swap if new model has comparable training samples
            if new_model.n_training_samples >= self._train_n_samples * 0.9:
                self._model = new_model
                self._last_trained = time.time()
                logger.info(
                    "FP retraining complete — new model: %d samples",
                    new_model.n_training_samples,
                )
                return True
            else:
                logger.warning(
                    "FP retraining produced degraded model (%d samples) — keeping previous",
                    new_model.n_training_samples,
                )
                return False

        except Exception as e:
            logger.exception("FP retraining failed: %s", e)
            return False
